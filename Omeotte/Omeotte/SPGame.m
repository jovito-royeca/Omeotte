...
/*
    ...
    textField.x = 0;//(background.width - textField.width) / 2;
    textField.y = 0;//(background.height / 2) - 135;
    [_contents addChild:textField];
    
    SPImage *image = [[SPImage alloc] initWithTexture:[SPMedia atlasTexture:@"sparrow"]];
...
- (void)onImageTouched:(SPTouchEvent *)event
{
    ...
    if ([touches anyObject]) [SPMedia playSound:@"sound.caf"];
}
